...
class Command(BaseCommand):
    help = 'Populates the database with random geneatred data.'
    ...
    def handle(self, *args, **options):
        ...
        for i in range(0, amount):
            giverName = generate_giver_name()
            giver = Giver.objects.get_or_create(name=giverName)
            feedback = Feedback.objects.create(
                activity=random.choice(Feedback.ACTIVITIES)[0],
                giver=Giver.objects.get(name=giverName),
